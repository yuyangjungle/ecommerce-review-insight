import { analyzeDataset, evaluatePromptVersions, normalizeDataset } from "./analysis-engine.js";

const SAMPLE_DATASETS = [
  {
    path: "./data/wireless-earbuds-demo.json"
  }
];
...
function renderCompare(compareResult) {
  ...
  grid.innerHTML = [compareResult.left, compareResult.right]
    .map((variant) => {
      const result = variant.result;
      return `
        <article class="compare-card">
          <div class="compare-head">
            <h3>${escapeHtml(result.metadata.workflow_label)}</h3>
            <span>${escapeHtml(variant.prompt_version)}</span>
          </div>
          <p>${escapeHtml(result.metadata.workflow_description)}</p>
...
function getSelectedDataset() {
  ...
  const selectedId = document.getElementById("dataset-select").value;
  return state.datasets.find((dataset) => dataset.id === selectedId)?.dataset || null;
}

async function loadDatasets() {
  const datasets = await Promise.all(
    SAMPLE_DATASETS.map(async (sample) => {
      const response = await fetch(sample.path);
      if (!response.ok) {
        throw new Error(`无法读取样例数据：${sample.path}`);
      }
      ...
      return {
        ...
        review_count: dataset.reviews.length,
        dataset
      };
    })
  );
...
async function runAnalysis() {
  ...
  if (!dataset) {
    throw new Error("未找到可分析的数据集。");
  }

  setStatus("正在运行分析...", "loading");
  const result = analyzeDataset(dataset, promptVersion);
  renderResult(result);
...
async function runCompare() {
  const dataset = getSelectedDataset();
  if (!dataset) {
    throw new Error("未找到可对比的数据集。");
  }

  setStatus("正在比较两套提示词工作流...", "loading");
  const result = evaluatePromptVersions(dataset, "v1", "v2");
